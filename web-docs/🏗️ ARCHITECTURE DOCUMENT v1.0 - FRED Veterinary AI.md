...
# 🏗️ ARCHITECTURE DOCUMENT v1.0 - FRED Veterinary AI Platform

**Architect:** Alex (BMAD)
**Data:** 06 de Dezembro de 2025
**Status:** 📄 COMPLETO
**Projeto:** Sistema All-in-One Veterinário com IA
...
| :-- | :-- |
| Versão | 1.0 FINAL |
| Data Criação | 06/12/2025 |
| Arch Decisões | 12 ADRs (detalhados) |
| Diagrama Principal | System Context + Component + Deployment |
| Tecnologias | 22 ferramentas + integrações |
...
│  │  - Rate Limiting (100 req/min brute-force)              │    │
│  │  - CORS handling                                        │    │
│  │  - SSL/TLS termination                                  │    │
│  │  - Gzip compression                                     │    │
│  └─────────────────────────────────────────────────────────┘    │
...
│  │ (Relational) │  │  (S3 Storage)│  │  (Cache)     │          │
│  │              │  │              │  │              │          │
│  │ Prisma ORM   │  │ Audio files  │  │ Sessions     │          │
│  │ (Query opt)  │  │ PDFs         │  │ Tokens       │          │
│  │              │  │ Images       │  │ Cache data   │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
│                                                                   │
│  Backup Layer:                                                   │
│  └─ Backblaze B2 (PostgreSQL daily dump + MinIO snapshots)      │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
     │              │              │
     ▼              ▼              ▼
...
│                                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │ OpenAI API   │  │ Google API   │  │ Stripe/      │          │
│  │              │  │              │  │ Pagar.me     │          │
│  │ - Whisper    │  │ - Gemini 2.5 │  │              │          │
│  │ - GPT-4o     │  │ - Grounding  │  │ Payment      │          │
│  │              │  │   Search     │  │ processing   │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
│                                                                   │
│  ┌──────────────┐  ┌──────────────┐                            │
│  │ Mailgun/     │  │ Sentry/      │                            │
│  │ Sendgrid     │  │ UptimeRobot  │                            │
│  │              │  │              │                            │
│  │ Email        │  │ Monitoring   │                            │
│  │ transacional │  │ & Alerting   │                            │
│  └──────────────┘  └──────────────┘                            │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
...
│  │ UI Layer (/app)                                                 │  │
│  │                                                                  │  │
│  │  ├─ (dashboard)         [Veterinarian/Admin/Attendant views]   │  │
│  │  ├─ (schedule)          [Agenda - dia/semana/mês]              │  │
│  │  ├─ (patients)          [Tutores/Pets CRUD]                    │  │
...
│  │  └─ (onboarding)        [Setup inicial Farejador]              │  │
│  │                                                                  │  │
│  │  Shared Components:                                             │  │
...
│  │  ├─ /api/consultations/  [CRUD + cancel + cancel reason]       │  │
│  │  ├─ /api/patients/       [Tutor + Pet CRUD]                    │  │
...
│  │                                                                  │  │
│  │  Middleware (Next.js):                                          │  │
│  │  ├─ middleware.ts        [Auth check, CORS, logging]           │  │
...
│  │  └─ validationService    [Zod schemas + business rules]         │  │
│  │                                                                  │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                            │                                           │
│  ┌────────────────────────────────────────────────────────────────┐  │
...
│  │                                                                  │  │
│  │  ├─ /jobs/fredar-journal.ts    [06:00 AM daily]               │  │
│  │  ├─ /jobs/reminders.ts          [Every 30 min during work]    │  │
│  │  ├─ /jobs/backup.ts             [02:00 AM daily]              │  │
│  │  └─ /jobs/cleanup.ts            [03:00 AM daily]              │  │
│  │                                                                  │  │
│  │  Run via: Coolify scheduler / GitHub Actions / node-cron       │  │
│  │                                                                  │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                        │
└──────────────────────────────────────────────────────────────────────┘
...
```
VETERINARIAN FLOW: Audio Upload → Prontuário
═══════════════════════════════════════════════════════════════════════

1. VET UPLOADS AUDIO
   ┌─────────────────────────────────────────────────────────┐
   │ Vet clica "Gravar/Upload" em consulta                  │
   │ - Seleciona arquivo .mp3/.wav/.m4a/.ogg/.webm (50MB)   │
   │ - Drag-drop ou file picker                              │
   │ - Validação file size + format (client-side)            │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
2. UPLOAD TO MINIO
   ...
   │ - FormData: file + consultationId                       │
   │ - Server validação size/format                          │
   │ - Progress bar (streaming upload)                       │
   │ - Armazena: MinIO/audios/{clinic}/{consultation}.mp3   │
   │ - Resposta: { audioId, status: 'processing' }          │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
3. TRIGGER WHISPER TRANSCRIPTION
   ┌─────────────────────────────────────────────────────────┐
   │ POST /api/transcription/generate (async)                │
   │ - Server: Lê arquivo MinIO                              │
   │ - Calls: OpenAI Whisper API (model: large)              │
   │ - Whisper timeout: 300 segundos (max 60min audio)       │
   │ - Output: { text: "...", language: "pt" }              │
   │ - Salva transcription em DB (Consultation.transcription)│
   │ - Notifica VET via browser + email ("Pronto!")         │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
4. GENERATE MEDICAL RECORD (GPT-4o)
   ┌─────────────────────────────────────────────────────────┐
   │ POST /api/records/generate-from-transcript              │
   │ - Inputs:                                               │
   │   - Transcription text                                  │
   │   - Pet context (nome, age, weight, allergies)         │
   │   - Previous records (últimas 3 consultas)             │
   │                                                         │
   │ - Crafted Prompt:                                       │
   │   "Extract these from veterinary consultation:         │
   │    - Chief complaint, symptoms, physical exam findings  │
   │    - Preliminary diagnosis, differentials              │
   │    - Tests ordered, medications prescribed             │
   │    - Client instructions, follow-up date               │
   │    Return as JSON: {...}"                              │
   │                                                         │
   │ - Calls: OpenAI GPT-4o API                              │
   │ - GPT timeout: 60 segundos                              │
   │ - Output: JSON estruturado                              │
   │ - Salva em cache Redis por 1 hora (retry safety)       │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
5. VET REVIEWS & CONFIRMS
   ┌─────────────────────────────────────────────────────────┐
   │ GET /records/review (pré-preenchido com IA data)        │
   │ - Formulário: Cada campo com checkbox                   │
   │   ☑️ Correto  |  ❌ Editar  |  ✕ Remover              │
   │                                                         │
   │ - Vet revisa, faz ajustes (editing inline)              │
   │ - Diagnosis obrigatório (validação)                     │
   │ - Mínimo 1 campo preenchido                             │
   │ - Signature: Data + Nome + CRM auto-filled              │
   │ - Botões: "Salvar", "Descartar", "Salvar+Receita"      │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
6. SAVE MEDICAL RECORD
   ...
   │ - Validação final (Zod schema)                          │
   │ - Cria MedicalRecord entry (relacionado Consultation)   │
   │ - Gera PDF (PrintJS ou react-pdf library)              │
   │ - Armazena: MinIO/records/{clinic}/{record}.pdf        │
   │ - Salva em DB: record.pdfPath                           │
   │ - Timestamp: signedAt = now()                           │
   │ - Permanente (não pode apagar, soft-delete apenas)      │
   │                                                         │
   │ - Notifica:                                             │
   │   - Tutor: Email "Prontuário disponível"               │
   │   - Admin: Log auditoria (quem, quando, dados)         │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
7. OPTIONAL: GENERATE PRESCRIPTION
   ...
   │ - Pre-preenche medications do MedicalRecord             │
   │ - Vet seleciona template: Simple/Controlled/Special     │
   │ - Vet revisa + assina                                   │
   │ - Salva Prescription entry + PDF                        │
   │ - Tutor recebe via email                                │
   └─────────────────────────────────────────────────────────┘

TIMELINE: Audio upload → Prontuário pronto ~5-7 minutes
...
```
FRED CHAT: "Qual foi o último exame do Rex?"
═══════════════════════════════════════════════════════════════════════

1. USER SENDS MESSAGE
   ┌─────────────────────────────────────────────────────────┐
   │ User: "Qual foi o último exame do Rex?"                │
   │ - Chat input (textarea) → sends to server               │
   │ - Context: clinicId, userId (via JWT)                  │
   │ - Message logged para auditoria                         │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
2. GROUNDING (DB SEARCH FIRST)
   ┌─────────────────────────────────────────────────────────┐
   │ FredarService.processMessage()                          │
   │ - Parse user input: Extract keywords "exam" + "Rex"    │
   │ - Query DB:                                             │
   │   SELECT * FROM pets WHERE name ILIKE '%Rex%'          │
   │   AND clinicId = $1                                    │
   │                                                         │
   │ - If found: Fetch last exam                             │
   │   SELECT * FROM medical_records mr                     │
   │   INNER JOIN consultations c ON mr.consultationId = c.id
   │   WHERE c.petId = $petId                               │
   │   AND mr.examination IS NOT NULL                        │
   │   ORDER BY c.scheduledDate DESC LIMIT 1                │
   │                                                         │
   │ - If NOT found: Ask "Desculpe, não encontrei pet 'Rex'"│
   │   Quer que eu... [agende novo?]"                       │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
3. CONTEXT BUILDING
   ...
   │     date: "2025-11-15",                                │
   │     vet: "Dra. Maria",                                  │
   │     findings: "Exame físico normal...",                │
   │     tests: "Hemograma: WBC normal"                      │
   │   }                                                     │
   │ }                                                       │
   │                                                         │
   │ Build prompt for Gemini:                                │
   │ "User asked: 'Qual foi o último exame do Rex?'        │
   │  Pet: Rex, 5 years old                                 │
   │  Last exam: 2025-11-15, Dra. Maria                     │
   │  Findings: [...]                                       │
   │  Please respond naturally in Portuguese about this      │
   │  exam. Keep response concise (2-3 sentences)."         │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
4. CALL GEMINI (NO HALLUCINATION)
   ...
   │ - System prompt: "You're FRED, veterinary assistant"   │
   │ - Messages: [                                           │
   │     { role: "user", content: "..." },                   │
   │     { role: "model", content: "..." },  // history      │
   │     { role: "user", content: prompt }   // current      │
   │   ]                                                     │
   │ - Temperature: 0.3 (mais focado, menos criativo)        │
   │ - Max tokens: 200 (respostas curtas)                    │
   │ - Timeout: 10s                                          │
   │ - Response: "O último exame do Rex foi em 15/11...     │
   │             encontrou-se tudo normal..."                │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
5. SEND QUICK ACTION BUTTONS
   ...
   │ ┌───────────────────────────────────────┐             │
   │ │ FRED:                                 │             │
   │ │ "O último exame do Rex foi em 15/11..│             │
   │ │  encontrou-se tudo normal..."         │             │
   │ │                                        │             │
   │ │ [🔍 Ver completo] [📋 Hist. completo]│            │
   │ │ [📅 Agendar consulta] [💊 Editar]    │            │
   │ └───────────────────────────────────────┘             │
   │                                                        │
   │ - Botões executam ações (abre modal, navega, etc)     │
   │ - Context mantido (pode fazer follow-up questions)    │
   │ - Histórico limitado a 10 mensagens (évita token bloat)
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
6. LOG & AUDIT
   ...
   │ Salva em FredarLog:                                     │
   │ {                                                       │
   │   clinicId, userId, type: "chat_message",              │
   │   query: "Qual foi o último exame do Rex?",            │
   │   response: "O último exame...",                       │
   │   metadata: {                                           │
   │     petsSearched: ["Rex"],                             │
   │     recordsFound: 1,                                   │
   ...
   │ }                                                       │
   │                                                        │
   │ Useful for: Auditing, improving prompts, analyzing usage
   └─────────────────────────────────────────────────────────┘
...
CRON JOB RUNS: Daily 06:00 AM (clinic timezone)
   ├─ Node cron or GitHub Actions
   └─ Triggered via Coolify scheduler

1. FETCH YOUR CLINIC DATA
   ┌─────────────────────────────────────────────────────────┐
   │ CompetitorService.getYourClinic(clinicId)               │
   │ - Query Google Maps: Your clinic name + location        │
   │ - Extract: Current rating, review count, last reviews   │
   │ - Extract: Recent posts (if Instagram/Facebook)         │
   │ - Compare with yesterday's snapshot                     │
   │ - Changes: Delta in rating, new reviews, new posts      │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
2. FETCH COMPETITORS DATA
   ┌─────────────────────────────────────────────────────────┐
   │ CompetitorService.updateCompetitors(clinicId)           │
   │ - Loop over monitored competitors (max 20)              │
   │ - For each:                                             │
   │   ├─ Google Maps: Rating + review count + recent reviews│
   │   ├─ Instagram: Latest posts (if handle known)          │
   │   │  - Extract: Likes, comments, engagement              │
   │   │  - Post date, caption length                         │
   │   ├─ Facebook: Page likes, recent posts                 │
   │   ├─ Website: Check if updated (optional, future)       │
   │   └─ Store deltas in DB (dailyTracking table future)    │
   │                                                         │
   │ - Use: Gemini Grounding Search (public web scraping)    │
   │ - Respect: robots.txt, rate limits                      │
   │ - Timeout: 60s per competitor (safety)                  │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
3. ANALYZE TRENDS & OPPORTUNITIES
   ...
   │                                                         │
   │ Calculate:                                              │
   │ - Your rating vs competitors average                    │
   │ - Your review growth rate                               │
   │ - New competitors entered market (5km radius)           │
   │ - Competitors with rating decrease (opportunity?)       │
   │ - Top performers by rating (what they do?)              │
   │ - Your weaknesses vs market leader                      │
   │                                                         │
   │ Suggestions:                                            │
   │ - "Seu concorrente X teve -0.5 rating queda.            │
   │   Considere campanha de avaliações positivas"           │
   │ - "Nenhum concorrente oferece [serviço X].              │
   │   Oportunidade de diferenciação"                        │
   │ - "Você está [X] reviews abaixo da média local.         │
   │   Meta: Solicitar 5 reviews este mês"                  │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
4. BUILD JOURNAL CONTENT
   ┌─────────────────────────────────────────────────────────┐
   │ Journal structure (4 sections):                          │
   │                                                         │
   │ ┌─ Section 1: SEU NEGÓCIO ─────────────────────┐       │
   │ │ "Você está em [bairro], com nota [4.8]        │       │
   │ │  Últimas 7 dias:                               │       │
   │ │  + 12 novas consultas agendadas                │       │
   │ │  + 15 pacientes completaram                    │       │
   │ │  + 2 novos reviews 5⭐"                        │       │
   │ └────────────────────────────────────────────────┘       │
   │                                                         │
   │ ┌─ Section 2: MERCADO LOCAL ───────────────────┐       │
   │ │ "Mudanças últimas 24h:                         │       │
   │ │ • 1 nova clínica abriu a 3km                   │       │
   │ │ • Clínica X: Rating ↑ de 4.5 → 4.7            │       │
   │ │ • Clínica Y: 3 novos reviews (média 4⭐)       │       │
   │ │ • Nenhuma atividade em telemedicina local"     │       │
   │ └────────────────────────────────────────────────┘       │
   │                                                         │
   │ ┌─ Section 3: TOP 3 CONCORRENTES ──────────────┐       │
   │ │ "Clínica A (4.9⭐, 120 reviews):               │       │
   │ │   - Último post Instagram: 2 dias, 45 likes   │       │
   │ │   - Website: Agora oferece telemedicina       │       │
   │ │   - Tendência: ↑ (favorável)                  │       │
   │ │                                                │       │
   │ │ Clínica B (4.3⭐, 80 reviews):                │       │
   │ │   - Último post: 5 dias atrás                 │       │
   │ │   - Review recente: '⭐ Service lento'        │       │
   │ │   - Tendência: ↓ (oportunidade!)"             │       │
   │ └────────────────────────────────────────────────┘       │
   │                                                         │
   │ ┌─ Section 4: AÇÕES RECOMENDADAS ──────────────┐       │
   │ │ "Próximas 48h:                                 │       │
   │ │ ✓ Clínica B com reviews negativos.             │       │
   │ │   → Foque em atendimento excelente             │       │
   │ │   → Solicitar reviews ao final consulta        │       │
   │ │                                                │       │
   │ │ ✓ Clínica A iniciou telemedicina.              │       │
   │ │   → Pesquise regulamentação CFMV               │       │
   │ │   → Considere oferecer também                  │       │
   │ │                                                │       │
   │ │ ✓ Sua nota: 0.1 pontos abaixo média.           │       │
   │ │   → Meta: +3 reviews 5⭐ esta semana"          │       │
   │ └────────────────────────────────────────────────┘       │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
5. SAVE TO DATABASE & EMAIL
   ┌─────────────────────────────────────────────────────────┐
   │ FredarJournal.create({                                  │
   │   clinicId,                                             │
   │   journalDate: today,                                   │
   │   clinicSection: JSON.stringify({ rating, changes }),  │
   │   localSection: JSON.stringify({ market_data }),        │
   │   competitorSection: JSON.stringify({ competitors }),   │
   │   opportunitiesSection: JSON.stringify({ actions })     │
   │ })                                                       │
   │                                                         │
   │ NotificationService.sendEmail({                         │
   │   to: admin@clinic.com,                                 │
   │   template: "fredar-journal",                           │
   │   data: { journal, clinic_name }                        │
   │ })                                                       │
   │                                                         │
   │ Response: Email enviado em <5s                          │
   └─────────────────────────────────────────────────────────┘
                            │
                            ▼
6. DISPLAY IN DASHBOARD
   ┌─────────────────────────────────────────────────────────┐
   │ GET /api/fredar/journal/latest                          │
   │ - Retorna: Última journal entry (today's date)          │
   │ - Cache: 1 hour (não refetch constantemente)            │
   │ - UI: Cards expandíveis por seção                       │
   │ - Gráfico: Rating histórico (30 dias)                   │
   │ - Botões: "Compartilhar", "Exportar PDF", "Ver histórico"
   │                                                         │
   │ GET /api/fredar/journal/history?days=30                 │
   │ - Retorna: Últimas 30 entradas                          │
   │ - Timeline view (clicável por data)                     │
   └─────────────────────────────────────────────────────────┘

KEY CHARACTERISTICS:
- Diário: Sempre mesma hora (06:00), não real-time
- Privado: Só admin acessa (dados sensíveis)
- Insightful: Não apenas dados, traz sugestões ação
- Gratuito: Incluído no plano (não consume quota IA muito)
...
| **TypeScript** | 5.0+ | Type safety | Previne bugs em produção, melhor IDE experience |
| **React** | 18+ | UI library | Modern hooks, concurrent features, estável |
| **Tailwind CSS** | 3.3+ | Styling | Utility-first, rápido desenvolvimento, design system |
...
| **Zustand** | 4.x | State management | Lightweight (2KB), não precisa Context hell |
...
| **Recharts** | 2.x | Charts | React components, responsive, competidor gráficos |
...
| API | Model | Função | Custo | Limite |
| :-- | :-- | :-- | :-- | :-- |
| **OpenAI Whisper** | large | Audio→Text (PT-BR) | \$0.006/min | 25 MB max, 60min max |
| **OpenAI GPT-4o** | gpt-4o | Prontuário geração | \$0.003 input, \$0.006 output (1K tokens) | 128K tokens context |
...
- Whisper: 500 consultations × 30min avg = 250 hours = R\$ 5.40
- GPT-4o: 500 records × 1500 tokens avg = R\$ 2.25
- Gemini: 10K FRED requests/day (free tier covers)
- **Total IA Cost: ~R\$ 10-15/mês** (muito sustentável)
...
| Tipo | Tecnologia | Função | Justificativa |
| :-- | :-- | :-- | :-- |
...
```
/clinics/{clinicId}/
├── audios/{consultationId}/
│   └── original.mp3              # Uploaded file
├── records/{recordId}/
│   └── signed.pdf                # Generated PDF
├── prescriptions/{prescriptionId}/
│   └── signed.pdf                # Prescription PDF
├── tutor-avatars/{tutorId}.jpg   # Profile pics
└── pet-photos/{petId}.jpg        # Pet photos
```

**Cost Analysis (1000 consultations/month):**

- Audio storage: 1000 × 25MB = 25GB → R\$ 0.15/mth
- PDFs: 1000 × 2MB = 2GB → R\$ 0.01/mth
...
├─ POST   /api/consultations/{id}/cancel  # Action endpoint
└─ DELETE /api/consultations/{id}         # Delete (soft-delete)
...
Request:
{
  message: "Qual foi o último exame do Rex?",
...
Response (200 OK):
{
  success: true,
  data: {
    message: "O último exame do Rex foi em 15/11...",
    conversationId: "uuid",
    actions: [
      {
        type: "button",
...
export async function fredarChat(
  ...
  if (!groundedContext.found) {
    return {
      message: `Desculpe, não encontrei "${groundedContext.searchTerm}". Quer que eu...?`,
      actions: [
...
└─ Overall Score: >85 mobile, >90 desktop

API Performance:
├─ Response time (p95): <200ms
├─ Database query (p95): <100ms
├─ File upload (1GB): <2 minutes
└─ Concurrent users: 100+

Scalability:
├─ Max consultations/day: 1,000+
...
describe('medicalRecordService.generateMedicalRecord', () => {
  it('should extract chief complaint from transcript', async () => {
    const transcript = 'O pet chegou vomitando há 2 dias'
...
- ✅ Foundational: Week 1-2 (setup, auth UI)
- ✅ Agenda: Week 3-4 (calendar, modals)
- ✅ Records: Week 5-6 (forms, history)
- ✅ FRED Chat: Week 7-8 (component, styling)
...
- ✅ Auth: Week 2-3 (NextAuth.js, JWT, RBAC)
- ✅ Consultation CRUD: Week 3-4 (API routes, validation)
- ✅ Transcription pipeline: Week 5-6 (Whisper integration)
- ✅ Medical records: Week 6-7 (GPT-4o, schema validation)
- ✅ FRED service: Week 7-8 (Gemini integration, grounding)
...
- ✅ VPS + Docker: Week 1 (Coolify setup)
- ✅ CI/CD: Week 2 (GitHub Actions)
- ✅ Monitoring: Week 11 (Sentry, uptime robot)
- ✅ Backup: Week 11 (CRON + B2)
...
- ✅ Complete system design (components, data flow, integrations)
- ✅ Technology stack justified (why each choice)
- ✅ Security \& compliance (LGPD, encryption, RBAC)
- ✅ Performance targets \& caching strategy
- ✅ Scalability path (up to 1000+ clinics)
- ✅ DevOps \& deployment (Docker, Coolify, CI/CD)
- ✅ Cost-effective infrastructure (~\$65/month base)
...
**Próximo:** PO (Product Owner) criar Master Checklist + Sharding
**Responsável:** User (Dono Digital Dog) validar com Architect antes de passar PO
